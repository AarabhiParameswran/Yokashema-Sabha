...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        const users: any[] = JSON.parse(localStorage.getItem('users')!) || [{ username: 'admin', mobileNumber: 'admin@themesbrand.com', password: '123456' }];
        ...
        return of(null).pipe(mergeMap(() => {
            ...
            if (request.url.endsWith('/users/authenticate') && request.method === 'POST') {
                const filteredUsers = users.filter(user => {
                    return user.mobileNumber === request.body.mobileNumber && user.otp=== request.body.password;
                });
                if (filteredUsers.length) {
                    ...
                    return throwError({ error: { message: 'Username or otpis incorrect' } });
                }
            }
            ...
            // get chatmessage
            if (request.url.endsWith('/app/mobileNumberData') && request.method === 'GET') {
                if (emailData) {
